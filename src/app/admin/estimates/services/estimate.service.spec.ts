...
describe('EstimateService', () => {
  ...
  it('loads public estimate without auth', () => {
    ...
    expect(api.get).toHaveBeenCalledWith('estimates/public/token-1', undefined, false);
  });

  it('loads public pdf without auth', () => {
    api.getBlob.and.returnValue(of(new Blob()));
    service.getPublicPdf('token-1').subscribe();
    expect(api.getBlob).toHaveBeenCalledWith('estimates/public/token-1/pdf', false);
  });
});